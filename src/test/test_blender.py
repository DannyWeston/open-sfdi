...
def test_calibration():
    with utils.ProcessingContext.UseGPU(True):
        ...
        camera = devices.FileCamera(resolution=(1920, 1080), channels=1, refresh_rate=30.0, 
            images=[img_repo.Get(str(i).zfill(4)) for i in range(48)],
            char=ch.ZhangChar(
        ...
        shifter_x = phase.NStepShifter([8, 8, 8])
        shifter_y = phase.NStepShifter([8, 8, 8])
        ...
        char_board = ch.Checkerboard(
            poi_count=(24, 14), square_size=(250.0/25.0, 250.0/15.0),
        )
        ...
        for i in range(num_char_poses):
            ...
            pois = char_board.find_pois(dc_img)
        ...
        camera.char.execute(char_board, pois_cam, camera.resolution)
        print(camera)
        ...
        projector.char.execute(
            char_board, pois_proj, projector.resolution,
            extraFlags=cv2.CALIB_FIX_PRINCIPAL_POINT
        )
        print(projector)
        
        # Jointly calibrate the system
        joint_char = camera.char.joint_char(projector.char, char_board)
        print(joint_char)
